...
const FIRECRAWL_BASE = process.env.FIRECRAWL_BASE || "https://api.firecrawl.dev/v2";
...
export function createWebSearchTool(sendMessage: (message: any) => void) {
  return tool({
    ...
    inputSchema: z.object({
      ...
      limit: z.number().int().min(1).max(50).optional().describe("Max number of results (default 3)"),
      scrape: z.boolean().optional().describe("If true, fetch full content for each result"),
      formats: z.array(z.string()).optional().describe("Formats for scraped content, e.g. ['markdown']"),
      sources: z.array(z.string()).optional().describe("sources: ['web','news','images']"),
      categories: z.array(z.string()).optional().describe("search categories, e.g. ['github','research']"),
      tbs: z.string().optional().describe("time-based search filter, e.g. 'qdr:d'"),
    }),
    async execute(input, { abortSignal }) {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Firecrawl search failed: ${res.status} ${res.statusText} ${txt}`);
      }
