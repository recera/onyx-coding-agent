...
export function createUrlExtractTool(sendMessage: (message: any) => void) {
  return tool({
    description: "Scrape a single URL with Firecrawl. Returns markdown/html/screenshots and metadata.",
    inputSchema: z.object({
      url: z.string().url(),
      formats: z.array(z.string()).optional().describe("formats like ['markdown','html','links']"),
      proxy: z.enum(["auto", "basic", "stealth"]).optional().describe("proxy strategy (stealth costs more credits)"),
      actions: z
        .array(z.record(z.string(), z.any()))
        .optional()
        .describe("optional actions array (click, wait, type) for interactive extraction"),
      timeout: z.number().int().min(1).optional().describe("timeout seconds for the scrape"),
    }),
    async execute(input, { abortSignal }) {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Firecrawl scrape failed: ${res.status} ${res.statusText} ${txt}`);
      }
